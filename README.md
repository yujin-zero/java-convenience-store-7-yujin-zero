...
1. Product : 상품 정보 및 재고 관리
    - [ ] 상품명과 상품 가격 반환
    - [ ] 현재 재고 수량 반환
    - [ ] 재고 감소
    - [ ] 프로모션 재고 여부 확인
    - [ ] 프로모션 재고 감소
2. CartItem : 장바구니에 담긴 단일 상품 정보 관리
    - [ ] 상품 정보와 구매 수량 반환
    - [ ] 총 상품 가격 계산
3. Cart : 장바구니 관리 및 구매 내역 요약
    - [ ] 장바구니에 아이템 추가
    - [ ] 장바구니에 담긴 모든 아이템 리스트 반환
    - [ ] 장바구니 전체 금액 계산
    - [ ] 장바구니에 담긴 전체 수량 계산
    - [ ] 프로모션 적용하여 할인 금액 계산
4. Promotion : 프로모션 할인 정책 관리
    - [ ] 해당 상품에 프로모션 적용 가능 여부 확인
    - [ ] 장바구니 아이템에 대한 프로모션 할인 금액 계산
    - [ ] 무료로 제공되는 증정 상품 수량 계산
5. MemebershipDiscount : 멤버십 할인 정책 관리
    - [ ] 멤버십 할인 금액 계산
    - [ ] 최대 할인 한도 반환
6. Receipt : 영수증 생성
    - [ ] 구매된 상품 정보 추가
    ...
    - [ ] 할인 내역 설정
    - [ ] 최종 결제 금액 설정
7. PaymentSystem : 결제 처리 및 전체 흐름 관리
    - [ ] 결제 시스템 초기화
    - [ ] 상품을 장바구니에 추가
    - [ ] 프로모션 및 멤버십 할인 적용
    - [ ] 장바구니 결제 처리
    - [ ] 영수증 생성
8. InputView : 사용자 입력 처리
    - [ ] 상품명 및 수량 입력 받기
    - [ ] 멤버십 여부 입력 받기
    - [ ] 추가 구매 여부 입력 받기
9. OutputView : 출력 처리
